...
| F-04a | Rolle `tutr_app`, `app.*`-Helper, `withActor()`, Policy-Runner, RLS-Metatest, Test-DB angebunden                                                                  | §8, §11     | doing  | Supavisor akzeptiert `tutr_app.<ref>` auf 6543 und 5432; kein Fallback nötig. Offen: `DATABASE_URL` in `.env.local` auf die neue Rolle umstellen, GitHub-Secrets setzen |
