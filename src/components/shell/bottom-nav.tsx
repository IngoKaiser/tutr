    ...
    >
      <ul className="mx-auto flex max-w-2xl">
        {SECTIONS.map((s) => {
          ...
          return (
            <li key={s.segment} className="flex-1">
              <Link
                href={s.href}
                ...
                className={`focus-visible:outline-koenigsblau flex flex-col items-center gap-1 px-1 py-3 text-xs transition-colors focus-visible:outline-2 focus-visible:outline-offset-[-2px] ${
                  isActive ? "text-koenigsblau font-semibold" : "text-tinte-leise hover:text-tinte"
                }`}
              >
                <span
                  aria-hidden="true"
                  className={`h-0.5 w-6 rounded-full ${isActive ? "bg-koenigsblau" : "bg-transparent"}`}
                />
                {s.label}
              </Link>
            </li>
